...
export default function EditWorkoutScreen() {
    ...
    const { customWorkouts, updateCustomWorkout } = useWorkoutStore();
    ...
    useEffect(() => {
        const existingWorkout = customWorkouts.find(w => w.id === id);
        if (existingWorkout) {
            setName(existingWorkout.name);
            setDescription(existingWorkout.description);
            setWorkoutImage(existingWorkout.imageUrl);
            setDifficulty(existingWorkout.difficulty);

            // Map existing exercises to our editable format, 
            // converting duration back to minutes for the form input
            setExercises(existingWorkout.exercises.map(ex => ({
                ...ex,
                duration: ex.duration ? ex.duration / 60 : 0
            })));
        } else {
            // If we somehow get here and the workout doesn't exist, go back
    ...
    const updateExerciseImage = (exerciseId: string, imageUrl: string) => {
        setExercises(exercises.map(ex =>
            ex.id === exerciseId ? { ...ex, imageUrl } : ex
        ));
    };
    ...
    const handleSave = () => {
        ...
        const updatedWorkout: Workout = {
            ...
            exercises: exercises.map(ex => ({
                ...
                imageUrl: ex.imageUrl || 'https://images.unsplash.com/photo-1541534741688-6078c6bfb5c5?w=800&q=80',
                notes: ex.notes || ''
            })),
            imageUrl: workoutImage || 'https://images.unsplash.com/photo-1517836357463-d25dfeac3438?w=800&q=80'
        };

        updateCustomWorkout(updatedWorkout);
        router.back();
    };
